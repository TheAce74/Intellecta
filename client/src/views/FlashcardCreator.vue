<template>
  <div>
    ...
    <form @submit.prevent="handleCreateFlashcards" class="mb-8">
      <div class="mb-4">
        <label for="file" class="block text-gray-700 font-bold mb-2">Upload File (PDF, DOCX, or TXT)</label>
        <input
          type="file"
          id="file"
    ...
    <div v-if="flashcards.length > 0" class="bg-white p-6 rounded-lg shadow-md">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div v-for="(flashcard, index) in flashcards" :key="index" class="bg-gray-100 p-4 rounded-lg">
          ...
          <div class="mt-2">
            ...
            <select v-model="flashcard.difficulty" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
...
const saveFlashcards = async () => {
  if (isLoggedIn.value && flashcards.value.length > 0) {
    ...
    // Show a success message or redirect to My Flashcards page
  } else {
